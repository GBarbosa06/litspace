...
const EditReview = () => {
  ...
  const {
    ...
    loading,
    error: fetchError,
  } = useFetchDocument("reviews", id);

  const {
    book,
    loading: bookLoading,
    error: bookError,
  } = useGoogleBookById(review?.bid);
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center sm:items-start justify-center mt-10 sm:mt-20 mb-20 sm:mb-30 gap-5">
      <div className="w-full sm:w-auto">
        {title && <h1 className="text-2xl sm:text-3xl font-bold">{title}</h1>}
        {!title && <h1 className="text-2xl sm:text-3xl font-bold">Livro</h1>}
        ...
        >
          <label className="my-5 w-full">
            ...
            <StarRating onRate={handleRating} initialRate={rating} />
          </label>
          ...
          <button onClick={handleSubmit} className="btn mb-5">
            Pronto!
          </button>
      ...
      {cover && (
        <img
          src={cover}
          alt={title}
          className="w-full sm:w-auto max-w-xs rounded"
        />
      )}
    </div>
  );
